...
欢迎使用 rstatatools，这个包里的函数并没有统一的目标，而是我平常使用的各种函数的集合，由于 R 包的开发不能使用中文，我又懒得写注释，所以就随便写写介绍，大家试着理解下。
...
+ `cn_ngrams()`: 中文文本的 n-grams 分词 token 函数，用于 tidyr::unnest_token() 函数里面；
+ `cnseg_tokenfun()`: 中文文本的分词 token 函数，用于 tidyr::unnest_token() 函数里面；
+ `cntrade()`: 不再可用；
